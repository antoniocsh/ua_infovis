###############################################################################
#       						Cone.py
###############################################################################

# This example creates a polygonal model of a Cone e visualize the results in a
# VTK render window.
# The program creates the cone, rotates it 360º and closes
# The pipeline  source -> mapper -> actor -> renderer  is typical 
# and can be found in most VTK programs

# Import all VTK modules
from vtkmodules.all import *

# Import only needed modules
# import vtkmodules.vtkInteractionStyle
# import vtkmodules.vtkRenderingOpenGL2
# from vtkmodules.vtkFiltersSources import vtkConeSource
# from vtkmodules.vtkRenderingCore import (
#     vtkActor,
#     vtkPolyDataMapper,
#     vtkRenderWindow,
#     vtkRenderWindowInteractor,
#     vtkRenderer
# )

def main():

    # We Create an instance of vtkConeSource and set some of its
    # properties. The instance of vtkConeSource "cone" is part of a
    # visualization pipeline (it is a source process object); it produces data
    # (output type is vtkPolyData) which other filters may process.
    
    sphereSource = vtkSphereSource()
    sphereSource.SetThetaResolution(8)
    sphereSource.SetPhiResolution(8)

    
    # We create an instance of vtkPolyDataMapper to map the polygonal data
    # into graphics primitives. We connect the output of the cone source 
    # to the input of this mapper.
  
    sphereMapper = vtkPolyDataMapper()
    sphereMapper.SetInputConnection(sphereSource.GetOutputPort())

    normalsFlat = vtkPolyDataNormals()
    normalsFlat.SetInputConnection(sphereSource.GetOutputPort())
    normalsFlat.ComputePointNormalsOff()
    normalsFlat.Update()

    mapperFlat = vtkPolyDataMapper()
    mapperFlat.SetInputConnection(normalsFlat.GetOutputPort())

    # We create an actor to represent the cone. The actor orchestrates rendering
    # of the mapper's graphics primitives. An actor also refers to properties
    # via a vtkProperty instance, and includes an internal transformation
    # matrix. We set this actor's mapper to be coneMapper which we created
    # above.

  
    sphereActor1 = vtkActor()
    sphereActor1.SetMapper(sphereMapper)
    sphereActor1.GetProperty().SetColor(1.0, 0.9, 0.8)
    sphereActor1.GetProperty().SetDiffuse(0.6)
    sphereActor1.GetProperty().SetSpecular(0.6)
    sphereActor1.GetProperty().SetSpecularPower(30)

    sphereActor2 = vtkActor()
    sphereActor2.SetMapper(mapperFlat)
    sphereActor2.GetProperty().SetColor(1.0, 0.9, 0.8)
    sphereActor2.GetProperty().SetDiffuse(0.6)
    sphereActor2.GetProperty().SetSpecular(0.6)
    sphereActor2.GetProperty().SetSpecularPower(30)    
    sphereActor2.GetProperty().SetInterpolationToFlat()


    
    sphereActor3 = vtkActor()
    sphereActor3.SetMapper(sphereMapper)
    sphereActor3.GetProperty().SetColor(1.0, 0.9, 0.8)
    sphereActor3.GetProperty().SetDiffuse(0.6)
    sphereActor3.GetProperty().SetSpecular(0.6)
    sphereActor3.GetProperty().SetSpecularPower(30)    
    sphereActor3.GetProperty().SetInterpolationToPhong()

    sphereActor4 = vtkActor()
    sphereActor4.SetMapper(sphereMapper)
    sphereActor4.GetProperty().SetColor(1.0, 0.9, 0.8)
    sphereActor4.GetProperty().SetDiffuse(0.6)
    sphereActor4.GetProperty().SetSpecular(0.6)
    sphereActor4.GetProperty().SetSpecularPower(30)
    sphereActor4.GetProperty().SetInterpolationToGouraud()

    # Create the Renderer and assign actors to it. A renderer is like a
    # viewport. It is part or all of a window on the screen and it is
    # responsible for drawing the actors it has.  We also set the background
    # color here.


    # left half 
    ren1 = vtkRenderer()
    ren1.AddActor(sphereActor1)
    ren1.SetBackground(0.1, 0.2, 0.4)
    ren1.SetViewport(0.0, 0.5, 0.5, 1.0)     

    # right half
    ren2 = vtkRenderer()
    ren2.AddActor(sphereActor2)
    ren2.SetBackground(0.2, 0.3, 0.4)
    ren2.SetViewport(0.5, 0.5, 1.0, 1.0)  

    ren3 = vtkRenderer()
    ren3.AddActor(sphereActor3)
    ren3.SetBackground(0.3, 0.4, 0.6)
    ren3.SetViewport(0.0, 0.0, 0.5, 0.5)

    ren4 = vtkRenderer()
    ren4.AddActor(sphereActor4)
    ren4.SetBackground(0.4, 0.5, 0.7)
    ren4.SetViewport(0.5, 0.0, 1.0, 0.5)

    # Finally we create the render window which will show up on the screen.
    # We put our renderer into the render window using AddRenderer. We also
    # set the size to be 300 pixels by 300.
    ren1.ResetCamera()
    ren2.ResetCamera()
    ren3.ResetCamera()
    ren4.ResetCamera()

    light = vtkLight()
    light.SetLightTypeToSceneLight()
    light.SetPosition(5, 2, 1)
    light.SetFocalPoint(0, 0, 0)
    light.SetColor(1.0, 1.0, 1.0)
    light.SetIntensity(1.0)

    ren1.AddLight(light)
    ren2.AddLight(light)
    ren3.AddLight(light)
    ren4.AddLight(light)

    renWin = vtkRenderWindow()
    renWin.SetSize(600, 300)

    renWin.AddRenderer(ren1)
    renWin.AddRenderer(ren2)
    renWin.AddRenderer(ren3)
    renWin.AddRenderer(ren4)

    renWin.SetWindowName("Two Viewports - Sphere Shading Options")

    
    # Adds a render window interactor to the cone example to
    # enable user interaction (e.g. to rotate the scene)
    iren = vtkRenderWindowInteractor()
    iren.SetRenderWindow(renWin)

    iren.Initialize()
    iren.Start()


if __name__ == '__main__':
    main()